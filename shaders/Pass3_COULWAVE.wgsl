...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (near_dry > 0. && periodic_bc_fix < 0)  //COULWAVE
    ...
    // Calculate eta stencil
        ...
        let B_up_right = textureLoad(txBottom, uprightIdx, 0).z;
        let B_down_right = textureLoad(txBottom, downrightIdx, 0).z;
        let B_up_left = textureLoad(txBottom, upleftIdx, 0).z;
        let B_down_left = textureLoad(txBottom, downleftIdx, 0).z;
        let h_up_right = eta_up_right - B_up_right;
        let h_down_right = eta_down_right - B_down_right;
        let h_up_left = eta_up_left - B_up_left;
        let h_down_left = eta_down_left - B_down_left;
    ...
    // replace with 4th order when dispersion is included
        ...
        let hv_up = in_state_up.z;
        let hv_down = in_state_down.z;
        let hv_right = in_state_right.z;
        let hv_left = in_state_left.z;
        let hv_up_right = in_state_up_right.z;
        let hv_down_right = in_state_down_right.z;
        let hv_up_left = in_state_up_left.z;
        let hv_down_left = in_state_down_left.z;

        let hu_up = in_state_up.y;
        let hu_down = in_state_down.y;
        let hu_right = in_state_right.y;
        let hu_left = in_state_left.y;
        let hu_up_right = in_state_up_right.y;
        let hu_down_right = in_state_down_right.y;
        let hu_up_left = in_state_up_left.y;
        let hu_down_left = in_state_down_left.y;

        let v_up = hv_up / h_north;
        let v_down = hv_down / h_south;
        let v_right = hv_right / h_east;
        let v_left = hv_left / h_west;
        let u_up = hu_up / h_north;
        let u_down = hu_down / h_south;
        let u_right = hu_right / h_east;
        let u_left = hu_left / h_west;
        let u_up_right = hu_up_right / h_up_right;
        let u_down_right = hu_down_right / h_down_right;
        let u_up_left = hu_up_left / h_up_left;
        let u_down_left = hu_down_left / h_down_left;
        let v_up_right = hv_up_right / h_up_right;
        let v_down_right = hv_down_right / h_down_right;
        let v_up_left = hv_up_left / h_up_left;
        let v_down_left = hv_down_left / h_down_left;

        let ux = 0.5 * (u_right - u_left) * globals.one_over_dx;
        let uy = 0.5 * (u_up - u_down) * globals.one_over_dy;
        let vx = 0.5 * (v_right - v_left) * globals.one_over_dx;
        let vy = 0.5 * (v_up - v_down) * globals.one_over_dy;
        let uxx = 0.5 * (u_right - 2.0 * u_here + u_left) * globals.one_over_d2x;
        let uxy = 0.5 * (u_up_right - u_down_right - u_up_left + u_down_left) * globals.one_over_dxdy;
        let vyy = 0.5 * (v_up - 2.0 * v_here + v_down) * globals.one_over_d2y;
        let vxy = 0.5 * (v_up_right - v_down_right - v_up_left + v_down_left) * globals.one_over_dxdy;

        let hux = 0.5 * (hu_right - hu_left) * globals.one_over_dx;
        let huy = 0.5 * (hu_up - hu_down) * globals.one_over_dy;
        let hvx = 0.5 * (hv_right - hv_left) * globals.one_over_dx;
        let hvy = 0.5 * (hv_up - hv_down) * globals.one_over_dy;
        let huxx = 0.5 * (hu_right - 2.0 * hu_here + hu_left) * globals.one_over_d2x;
        let huxy = 0.5 * (hu_up_right - hu_down_right - hu_up_left + hu_down_left) * globals.one_over_dxdy;
        let hvyy = 0.5 * (hv_up - 2.0 * hv_here + hv_down) * globals.one_over_d2y;
        let hvxy = 0.5 * (hv_up_right - hv_down_right - hv_up_left + hv_down_left) * globals.one_over_dxdy;
        ...
        let temp2 = 1.0 / 6.0 * (eta_here*eta_here*eta_here + d_here*d_here*d_here ) - 0.5*za*za*h_here;
	    let temp3 = 0.5*(eta_here*eta_here - d_here*d_here) - za * h_here;
	    let E1_x = temp2 * dSdx + temp3 * dTdx;
	    let E1_y = temp2 * dSdy + temp3 * dTdy;
    ...
    let source_term = vec4<f32>(dhdt + overflow_dry, -globals.g * h_here * detadx - in_state_here.y * friction_ + breaking_x + (Psi1x + Psi2x) + press_x, -globals.g * h_here * detady - in_state_here.z * friction_ + breaking_y + (Psi1y + Psi2y) + press_y, hc_by_dx_dx + hc_by_dy_dy + 2.0 * hc_by_dx_dy + c_dissipation);
