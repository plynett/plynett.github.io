...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let cm = 0.2; // Smag coef
    // 2.*strain rate tensor = 4*u_x^2 + 4*vy^2 +4*ux*vy + (uy + vx)^2
    let u = textureLoad(txU, idx, 0);
    let v = textureLoad(txV, idx, 0);
    let u_x = (u.y - u.w) * globals.one_over_dx;
    let v_x = (v.y - v.w) * globals.one_over_dx;
    let u_y = (u.x - u.z) * globals.one_over_dy;
    let v_y = (v.x - v.z) * globals.one_over_dy;
    let nuH_Smag = cm*globals.dx*globals.dy*sqrt(4*u_x*u_x + 4*vy*vy +4*ux*vy + (uy + vx)*(uy + vx));
    // assume gradients in nu are not important right now
    let temp1 = nuH_Smag * u_x;
    let temp2 = nuH_Smag * u_y;
    let temp3 = nuH_Smag * (u_x + v_x);
