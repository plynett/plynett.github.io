...
fn fs_main(@location(1) uv: vec2<f32>) -> FragmentOutput {
 ...
 //       render_surface = textureSample(depostion_Sed, textureSampler, uv).r - textureSample(erosion_Sed, textureSampler, uv).r; 
    ...
    if (photorealistic == 1) {  // add lighting for photo-realistic view
        ...
        let shininess = 320.0; // Shininess factor for specular highlight
        ...
        // Normal vector calcs
        var roughnessFactor = 0.002 * min(H, globals.base_depth / 100.);
    ...
    if (bottom + globals.delta >= waves && surfaceToPlot != 6 && design_component_allowed_on_land == 0) {
        ...
        if (photorealistic == 0) {
            ...
            let color_shallow = vec3<f32>(0.2, 0.45, 0.45); // shallow water ocean color
            let color_deep = color_shallow; //vec3<f32>(0.0, 0.25, 0.5); // deep ocean color
    ...
    breaking_texture = 0.5*breaking_texture; // tone down breaking texture effect
