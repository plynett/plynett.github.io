...
async function initializeWebGPUApp(configContent, bathymetryContent, waveContent) {
    ...
    // initalize Babylon.js engine
    var canvas_Babylon = document.getElementById('renderCanvas');
    const engine = new BABYLON.WebGPUEngine(canvas_Babylon);
    await engine.initAsync();
    var scene = new BABYLON.Scene(engine); // Create a basic BABYLON Scene object
    scene.clearColor = new BABYLON.Color4(0.5, 0.5, 0.5, 1); // Grey background

    // Camera
    var camera = new BABYLON.ArcRotateCamera("camera", 0.0, 0.0, 1.0, new BABYLON.Vector3(0, 0, 0), scene);  // init with default values
    camera.attachControl(canvas_Babylon, true);    
    camera.alpha = -Math.PI / 2;
    camera.beta = Math.PI / 2;
    camera.radius = 0.5*Math.max(calc_constants.WIDTH, calc_constants.HEIGHT);
    camera.setTarget(new BABYLON.Vector3(0, 0, 0)); // center of image.
    ...
    light.specular = new BABYLON.Color3(1, 1, 1); // White highlights

    // Create custom shader material
    const vertexShaderCode_Babylon = await fetchShader('/shaders/Babjs_vertex.fx');
    const fragmentShaderCode_Babylon = await fetchShader('/shaders/Babjs_frag.fx');
    
    var customMaterial = new BABYLON.ShaderMaterial("custom", scene, {
        vertex: "vertexShaderCode_Babylon",
        fragment: "fragmentShaderCode_Babylon",
        // Use vertexSource and fragmentSource to directly pass shader strings
        vertexSource: vertexShaderCode_Babylon,
        fragmentSource: fragmentShaderCode_Babylon,
    }, {
        attributes: ["position", "uv"],
        uniforms: ["worldViewProjection", "textureSampler"]
    });    

    // Load your texture - test
  ...
  // Create a plane and apply the material
    var plane = BABYLON.MeshBuilder.CreatePlane("plane", {width: calc_constants.WIDTH, height: calc_constants.HEIGHT}, scene);
