#!/usr/bin/env python3
"""MATLAB-generate, browser-run, and MATLAB-process NTHMP BP06 cases."""
...
def run_dir(case_id: str) -> Path:
    return ROOT / "runs" / "BP06" / f"{case_id}_solitary_disturbance"
...
def main() -> int:
    ...
    if not args.skip_generate:
        run_matlab(f"nthmp_generate('BP06', '{args.case}')")
    ...
    for case_id in selected_cases(args.case):
        ...
        if not args.skip_process:
            ...
            run_matlab(f"nthmp_process('BP06', {matlab_string(case_dir)}, {allow_missing}); nthmp_summary")
