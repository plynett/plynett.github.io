...
ref_ts_file = fullfile(ref_dir,'ts2cnew1.txt');
ref_runup_file = fullfile(ref_dir,'run2c.txt');
...
for p = 1:length(plot_gauges)
    ...
    xlabel('time*(g/d)^{0.5}')
    ylabel('\eta/d')
    title(sprintf('BP06 Case C %s',gauge_label{n}))
    axis([0 70 -0.2 .3])
...
writetable(runup_table,fullfile(analysis_dir,'bp06_case_c_matlab_runup_by_angle.csv'));
...
xlabel('angle (deg)')
ylabel('R / d')
title('BP06 Case C runup by angle')
legend('Laboratory','Celeris','Location','best')
saveas(gcf,fullfile(figure_dir,'bp06_case_c_runup_by_angle.png'))
