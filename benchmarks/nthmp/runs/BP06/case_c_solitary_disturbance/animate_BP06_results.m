...
for k = 1:length(elev_files)
    ...
    clf
    plot(x,bathy_line,'k-','LineWidth',1.5)
    hold on
    plot(x,eta_line,'b-','LineWidth',1.2)
    grid on
    xlim([0 29.3])
    ylim([-0.4 0.7])
    xlabel('x (m)')
    ylabel('z (m)')
    title(sprintf('BP06 Case C Celeris centerline surface, frame %d, t = %.2f s', ...
        frame_number(k),model_time))
    legend('Bathy/topo','Free surface','Location','best')
    drawnow
    pause(0.1)
end
