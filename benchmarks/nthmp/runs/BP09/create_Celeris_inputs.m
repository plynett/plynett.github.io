% Script to create BP09 nested-grid input files for WebGPU Celeris.
% This follows the Pearl Harbor master_nested_config.m workflow: define the
% grid hierarchy up front, loop through each grid, write config.json,
% bathy.txt, waves.txt, and etaInitCond.txt on each grid.

clear all
close all
clf

% High-Level Nested Grid Control Parameters
num_grids = 4; % A parent grid, B child grid, and two final C grids
base_refinement = 4; % refines the PMEL 450 m A-grid to about 112 m
child_refinement = 4; % parent-child refinement factor, giving final grids near 7 m
nested_dt = 10.0; % time step for nested boundary time-series output
nested_eta_threshold = 0.01; % eta threshold for trimming quiet nested-boundary samples
sim_duration = 3600.0; % total simulation duration for each BP09 grid

script_dir = fileparts(mfilename('fullpath'));
nthmp_dir = fileparts(fileparts(script_dir));
...
write_grid_extent_plot(resolved, sources, source_cache, reference_dir, output_root);
...
for grid_idx = 1:numel(grids)
    ...
    if isfield(grid, 'initial_condition_source') && ~isempty(grid.initial_condition_source)
        [eta, source_cache, eta_coverage] = interpolate_from_sources(source_list(grid.initial_condition_source), sources, source_cache, reference_dir, grid.x, grid.y, 0.0);
    end
    ...
    config = build_config(grid, bottom, rectangles, nested_dt, nested_eta_threshold, sim_duration);
    ...
    ic_path = fullfile(out_dir, 'IC.txt');
    if isempty(eta)
        if exist(eta_path, 'file'), delete(eta_path); end
        if exist(ic_path, 'file'), delete(ic_path); end
    else
        write_grid_txt(eta_path, eta);
        write_grid_txt(ic_path, eta);
    end
...
grids = make_grid('grid_a', 'gridA_okushiri', 'A', '', '', {'grid_b'}, ...
    ...
    {'pmel_a'}, 'pmel_i_a', {'iwanai', 'esashi', 'aonae'}, empty_pmel_link());
grids(end + 1) = make_grid('grid_b', 'gridB_south_okushiri', 'B', 'grid_a', 'gridB', {'grid_c_aonae', 'grid_c_monai'}, ...
    bounds_struct(139.0 + 23.22 / 60.0, 42.0 + 0.16 / 60.0, 139.0 + 40.0 / 60.0, 42.0 + 17.0 / 60.0), '', child_refinement, ...
    {'pmel_b1', 'pmel_b2', 'pmel_b3', 'pmel_a'}, {'pmel_i_b1', 'pmel_i_b2', 'pmel_i_b3', 'pmel_i_a'}, {'aonae', 'aonae_lighthouse', 'monai_camping', 'monai_river'}, pmel_link('grid_a', 165, 365));
grids(end + 1) = make_grid('grid_c_aonae', 'gridC_aonae', 'C_Aonae', 'grid_b', 'gridC_aonae', {}, ...
    bounds_struct(139.0 + 26.0 / 60.0, 42.0 + 2.5 / 60.0, 139.0 + 30.0 / 60.0, 42.0 + 5.0 / 60.0), '', child_refinement, ...
    {'pmel_c1'}, 'pmel_i_c1', {'aonae', 'aonae_lighthouse'}, pmel_link('grid_b', 27, 30));
grids(end + 1) = make_grid('grid_c_monai', 'gridC_monai', 'C_Monai', 'grid_b', 'gridC_monai', {}, ...
    ...
    {'pmel_c23'}, 'pmel_i_c23', {'monai_camping', 'monai_river'}, pmel_link('grid_b', 14, 49));
end
...
sources.pmel_b2 = pmel_source('PMEL/BATHYMETRY/D94-85-150m.txt', 94, 85, 139.0 + 30.0 / 60.0, 42.0 + 10.0 / 60.0, 139.0 + 40.0 / 60.0, 42.0 + 17.0 / 60.0);
sources.pmel_b3 = pmel_source('PMEL/BATHYMETRY/D61-49-150m.txt', 61, 49, 139.0 + 23.22 / 60.0, 42.0 + 10.0 / 60.0, 139.0 + 30.0 / 60.0, 42.0 + 14.0 / 60.0);
...
sources.pmel_i_a = pmel_initial_source('PMEL/INITIAL-WAVE/I379-687-450m-17a.txt', region_a.width, region_a.height, region_a.lon_min, region_a.lat_min, region_a.lon_max, region_a.lat_max);
sources.pmel_i_b1 = pmel_initial_source('PMEL/INITIAL-WAVE/I154-124-150m-17a.txt', 154, 124, 139.0 + 23.22 / 60.0, 42.0 + 0.16 / 60.0, 139.0 + 40.0 / 60.0, 42.0 + 10.0 / 60.0);
sources.pmel_i_b2 = pmel_initial_source('PMEL/INITIAL-WAVE/I94-85-150m-17a.txt', 94, 85, 139.0 + 30.0 / 60.0, 42.0 + 10.0 / 60.0, 139.0 + 40.0 / 60.0, 42.0 + 17.0 / 60.0);
sources.pmel_i_b3 = pmel_initial_source('PMEL/INITIAL-WAVE/I61-49-150m-17a.txt', 61, 49, 139.0 + 23.22 / 60.0, 42.0 + 10.0 / 60.0, 139.0 + 30.0 / 60.0, 42.0 + 14.0 / 60.0);
sources.pmel_i_c1 = pmel_initial_source('PMEL/INITIAL-WAVE/I112-94-50m-17a.txt', 112, 94, 139.0 + 26.0 / 60.0, 42.0 + 2.5 / 60.0, 139.0 + 30.0 / 60.0, 42.0 + 5.0 / 60.0);
sources.pmel_i_c23 = pmel_initial_source('PMEL/INITIAL-WAVE/I40-202-50m-17a.txt', 40, 202, 139.0 + 24.5 / 60.0, 42.0 + 4.0 / 60.0, 139.0 + 26.0 / 60.0, 42.0 + 9.5 / 60.0);
end
...
function s = pmel_initial_source(path, width, height, lon_min, lat_min, lon_max, lat_max)
s.kind = 'pmel_initial';
s.path = path;
s.width = width;
s.height = height;
...
function keys = source_list(value)
if iscell(value)
    keys = value;
else
    keys = {value};
...
switch spec.kind
    ...
    case 'pmel_initial'
        eta = read_pmel_grid(full_path, spec.width, spec.height);
        source.key = source_key;
        source.path = full_path;
        source.x = linspace(spec.lon_min, spec.lon_max, spec.width);
        source.y = linspace(spec.lat_min, spec.lat_max, spec.height);
...
function write_grid_extent_plot(resolved, sources, source_cache, reference_dir, output_root)
[source, ~] = load_source('pmel_a', sources, source_cache, reference_dir);
figure(1)
clf
imagesc(source.x, source.y, source.values)
set(gca, 'YDir', 'normal')
hold on
axis equal tight
colormap(turbo)
...
ylabel('latitude (deg)')
title('BP09 PMEL bathymetry and nested grid extents')

colors = lines(numel(resolved));
for k = 1:numel(resolved)
    grid = resolved{k};
    x_box = [grid.lon_ll grid.lon_ur grid.lon_ur grid.lon_ll grid.lon_ll];
    y_box = [grid.lat_ll grid.lat_ll grid.lat_ur grid.lat_ur grid.lat_ll];
    plot(x_box, y_box, '-', 'Color', colors(k,:), 'LineWidth', 1.8)
    text(grid.lon_ll, grid.lat_ur, ['  ' grid.label], 'Color', colors(k,:), ...
        'FontWeight', 'bold', 'VerticalAlignment', 'bottom')
end

analysis_dir = fullfile(output_root, 'analysis');
if ~exist(analysis_dir, 'dir'), mkdir(analysis_dir); end
saveas(gcf, fullfile(analysis_dir, 'bp09_grid_extents.png'))
end
...
[grid.x, grid.dx] = axis_from_bounds(grid.bounds.lon_min, grid.bounds.lon_max, grid.dx);
[grid.y, grid.dy] = axis_from_bounds(grid.bounds.lat_min, grid.bounds.lat_max, grid.dy);
...
function [axis_values, actual_spacing] = axis_from_bounds(start_value, end_value, target_spacing)
count = round((end_value - start_value) / target_spacing) + 1;
axis_values = linspace(start_value, end_value, count);
actual_spacing = axis_values(2) - axis_values(1);
end
...
function config = build_config(grid, bottom, rectangles, nested_dt, nested_eta_threshold, sim_duration)
config = struct();
config.grid_type = 2;
...
config.loadetaIC = double(~isempty(grid.initial_condition_source));
...
config.trigger_writeWaveHeight_time = sim_duration-1;
...
if ~isempty(eta)
    manifest.files.initial_condition = 'IC.txt';
