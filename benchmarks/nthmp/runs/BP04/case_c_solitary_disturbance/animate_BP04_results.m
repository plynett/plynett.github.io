% Animate BP04 Celeris-WebGPU surface output as x-z transects.
%
...
% For BP04, the benchmark x/d coordinate is shifted relative to the Celeris
% grid_type=1 coordinate.  Read the shift from the generated run config.
config = jsondecode(fileread(fullfile(pwd,'config.json')));
d = config.base_depth;
g = config.g;
profile_time_scale = sqrt(d/g);
benchmark_left_x_over_d = config.benchmark_left_x_over_d;
...
for k = 1:length(elev_files)
    ...
    grid on
    xlim([benchmark_left_x_over_d 35])
    ylim([-1.1 0.7])
    xlabel('x / d')
    ylabel('z / d')
    title(sprintf('BP04 Case C Celeris surface, frame %d, t = %.2f', ...
        frame_number(k),model_time/profile_time_scale))
