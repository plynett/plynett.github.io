...
dx = 0.015; % uniform grid size to use in model - the loaded bathy files will be interpolated to this grid size
dy = 0.015;
...
min_allowable_depth = 0.0000001; % min allowable water depth
infiltrationRate = 0.00;  % infiltration rate - for drying inundated cells
useBreakingModel = 0; % Case A is below the breaking threshold
algochanges=1; % 1=use new moving shoreline algorithm


% Benchmark Problem 4, Case A setup
% The laboratory case uses d=0.30 m, H/d=0.0185, and the same 1:19.85
% beach as BP01.  The left boundary is x=0 in Celeris.  The benchmark
% coordinate at that same location is x/d=-5.
base_depth = 0.30;
...
add_Disturbance = 1;
disturbanceType = 1;
disturbanceXpos = (38.34250117739536 - benchmark_left_x_over_d)*base_depth; % Celeris x for the benchmark initial solitary wave
...
maxdurationTimeSeries = 14;  % this is the time chunk to display and write discrete parts of the time series
...
% Case A reference profiles are at t/sqrt(d/g) = 30, 40, 50, 60, and 70.
% Start one interval before the first requested profile time so the written
% surfaces align with the reference profiles.
profile_times = [1 70];
profile_dt = 0.5;
...
trigger_writesurface_end_time = (profile_times(end)+0.5*profile_dt)*profile_time_scale;  % end time of surface write trigger
...
grid on
title('BP04 Case A Bathy/Topo Transect')
xlabel('Celeris x (m)')
